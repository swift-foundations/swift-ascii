...
extension Binary.ASCII.Parsing.Whole {
    ...
    public func call(_ bytes: [UInt8]) throws(Either<P.Failure, Binary.ASCII.Parsing.Error>) -> P.Output {
        try Binary_Parser_Primitives.Binary.Bytes.withInput(bytes) { (input: inout Binary_Parser_Primitives.Binary.Bytes.Input) throws(Either<P.Failure, Binary.ASCII.Parsing.Error>) -> P.Output in
            let value: P.Output
            do throws(P.Failure) {
